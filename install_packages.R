...
if("mlrMBO" %in% rownames(installed.packages()))
{
  ...
  devtools::install_github("mlr-org/mlrMBO", upgrade=c("never"))
}
if("destiny" %in% rownames(installed.packages()))
{
  print("destiny is installed")
} else {
  devtools::install_github("theislab/destiny", upgrade=c("never"))
}
if("UCell" %in% rownames(installed.packages()))
{
  print("UCell is installed")
} else {
  devtools::install_github("carmonalab/UCell", upgrade=c("never"))
}
# 5.3 ArchR ####
if("ArchR" %in% rownames(installed.packages()))
{
  print("ArchR is installed")
}  else { devtools::install_github("GreenleafLab/ArchR", ref="dev", repos = BiocManager::repositories(), upgrade = c("never"))
